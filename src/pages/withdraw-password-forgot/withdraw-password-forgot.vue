<template lang="pug">
    view(style="padding:10rpx 32rpx 0;")
        ...
        view(class="bb1 item df ai-center")
            view(class="itemL") 新提现密码:
            input(class="inp" v-model="password" placeholder="请输入6位纯数字" type="password" maxlength='6')
        view(class="bb1 item df ai-center")
            view(class="itemL") 确认密码:
            input(class="inp" v-model="password2" placeholder="请再次输入新提现密码" type="password" maxlength='6')
...
export default {
    ...
    methods: {
        toSendCode() {
            ...
            let params = { mobile: this.mobile, type: 22 };
        ...
        toSubmit() {
            ...
            if (!util.checkWithdrawPassword(this.password)) {
            ...
            http.request(urls.FORGOT_PAY_PASSWORD, "PUT", params).then(
                data => {
                    util.showToast('重置成功')
                    setTimeout(() => {
                        util.hideLoadingDialog();
                        util.linkto('usercenter');
                    }, 1000);
...
<style lang="stylus">
    @import "./withdraw-password-forgot.styl"
</style>